...
class iq(commands.Cog):
    ...
    async def iq(self, ctx, *,user: nextcord.User=None):
        ...
        if user.id in [846298981797724161, 818236132578820126]:
            iq_size = 420.00
        colour_hex = '%02x%02x%02x' % ( int((iq_size/4.2)*2.55), int((iq_size/4.2)*2.51), int((iq_size/4.2)*1.91) )
        colour = int(colour_hex, 16)
        embed=nextcord.Embed(description=f"{user.mention} IQ is {iq_size:.2f}.", color=colour)
