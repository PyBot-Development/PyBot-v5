...
class tts_slash(commands.Cog):
    def __init__(self, client):
        self.client = client
        
    @checks.default()
    @cooldown(1, support.cooldown, BucketType.user)
    @commands.slash_command(description=support.getDescription("en.json", "tts"))
    async def tts(self, ctx, *, text):
        async with ctx.typing():
            lang = support.getLanguageFileG(ctx.guild)
            max = 1000
            if len(str(text)) > max:
                await ctx.respond(embed=discord.Embed(description=lang["commands"]["tts"]["maxLenght"].format(max=max), color=support.colours.default))
                return
            text = text.split("-l")
            text.append("en")
            try:
