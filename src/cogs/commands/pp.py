...
class pp(commands.Cog):
    ...
    async def pp(self, ctx, *, user: nextcord.User=None):
        ...
        try:
            embed=nextcord.Embed(description=f"{user} pp size is {ppsize:.2f}cm/{ppsize_inch:.2f}inch.", color=colour)
        except:
            embed=nextcord.Embed(description=f"{user} pp size is {ppsize}cm/{ppsize_inch}inch.", color=colour)
